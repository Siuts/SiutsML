        ...
        for counter, rec in enumerate(selected_recordings):
            ...
            if len(rec_segments) > 0 and len(labels) > 0:
                ...
                if len(all_segments) == 0:
                    all_segments = rec_segments
                else:
                    all_segments = np.concatenate((all_segments, rec_segments), axis=0)
                all_labels = all_labels + labels
                all_rec_ids = all_rec_ids + rec_ids

                specimen = rec.get_name()
                if specimen in segments_count:
                    segments_count[specimen] += len(rec_segments)
...
def main():
    ...
    for specimen in siuts.species_list:
        recordings = sorted([x for x in plutof_recordings if x.get_name() == specimen and
                             x.segments_count is not None and x.segments_count >= 2],
            ...
            for counter, rec in enumerate(specimen_files):
                ...
                if len(rec_segments) > 0 and len(labels) > 0:
                    ...
                    if counter == 0:
                        ...
                        all_segments = np.vstack((all_segments, rec_segments))
